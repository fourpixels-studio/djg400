...
def view_order(request, order_number):
    try:
        ...
        if order.paid == True and order.is_email_sent == False:
            ...
            order.receipt.save(
                f'order_{order.get_order_number}.pdf',
                ContentFile(pdf_output), save=True
            )
            order.save()
            ...
            order.save()
            messages.success(
                request, "Thank you for your order! A confirmation email with your order details and receipt will be sent to you shortly.")
